const CERTIFICATIONS = [
  {
    name: "Meta Back End Developer Specialization",
    tag: "Meta",
    description:
      "9-course program covering Python, Django, REST APIs, system design, authentication, and backend deployment.",
    link: "https://www.coursera.org/account/accomplishments/professional-cert/PRTTZGRP0BN5",
    photo: "/Images/Certifications/meta_back_end.jpeg",
  },
  {
    name: "API Development Certification",
    tag: "Meta",
    description:
      "Hands-on experience building, testing, and deploying RESTful APIs.",
    link: "https://www.coursera.org/account/accomplishments/verify/BNGSH8W41C4E",
    photo: "/Images/Certifications/api_course.jpeg",
  },
  {
    name: "Quickstart: LangSmith Essentials",
    tag: "LangChain",
    description:
      "LLM tracing, evaluation pipelines, and prompt engineering for production-ready AI applications.",
    link: "https://academy.langchain.com/certificates/c4riphsjx3",
    photo: "/Images/Certifications/langsmith.jpg",
  },
  {
    name: "White Paper Contest — Certificate of Participation",
    tag: "Pune International Centre",
    description:
      "Authored and presented on using AI as a protector against the cybersecurity threats it generates.",
    link: "#",
    photo: "/Images/Certifications/white_paper.jpeg",
  },
  {
    name: "Meta React Basics Certificate",
    tag: "Meta",
    description:
      "Completed Meta's React Basics course, mastering reusable components, event handling, data management, routing, and building efficient, dynamic web applications.",
    link: "https://www.coursera.org/account/accomplishments/certificate/DI4PL5O2KR1G",
    photo: "/Images/Certifications/react_basics.jpeg",
  }
];

export default CERTIFICATIONS;
